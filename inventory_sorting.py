...
def empty_slot_filter(file):
    ...
    df = df[~(df["Location"].str.strip().str.contains(r"(?:HVA|PPP|KANI|OE|OEP|OEC|INTHP|DOMPAL|DOMPAO|WIP|QC|CAP|PK|PKP|WIRE)"))]
    ...
    df["Entry Status"] = np.where(~(df["Location"].str.strip().str.contains(r"^[A-Z]{1,3}[0-9]{0,1}\.[0-9]{2,3}\.(?:[0-9][A-Z]|[A-Z]{2})\.[0-9]{2}$", regex = True)), "Invalid", "Valid")

    # Identify locations that correspond to rack locator codes
    df["Rack Locations"] = np.where(df["Location"].str.strip().str.contains(r"(?:AA|BB|CC|QQ|RR|TT|XX|YY|ZZ|[B-H]{1}|[J-Z]{1}|[B-H]{1}[0-9]{1}|[J-Z]{1}[0-9]{1})\.[0-9]{2}\.[0-9][A-Z]\.[0-9]{2}", regex = True), "Rack",
        np.where(df["Location"].str.strip().str.contains(r"(?:[A-Z]EC|SPS|WMPS|PS|SMI|SEB)\.[0-9]{2,3}\.[0-9][A-Z]\.[0-9]{2}", regex = True), "Rack", "Non-Rack"))
    ...
    df["Floor Locations"] = np.where(df["Location"].str.strip().str.contains(r"(?:L|M|S)\.00\.00.[0-9]{2}", regex = True), "Floor",
        np.where(df["Location"].str.strip().str.contains(r"(?:REC|UU|DOOR\.FRONT|SEB)", regex = True), "Floor",
            np.where(df["Location"].str.strip().str.contains(r"\bI\.[0-9]{2}\.[0-9]{1,2}[A-Z]{0,1}\.0[1-9]{1}", regex = True), "Floor",
                np.where(df["Location"].str.strip().str.contains(r"\bA\.[0-9]{2}\.[0-9][A-Z]\.[0-9]{2}", regex = True), "Floor", "Non-Floor"))))

    # Create a new column to identify each locator code as either a rack space, floor space, or floor/rack space (SEB locators)
    # If none, locator code will be set as "Other"
    df["Inventory Type"] = np.where((df["Floor Locations"] == "Floor") & (df["Rack Locations"] == "Rack"), "Floor/Rack",
        np.where(df["Floor Locations"] == "Floor", "Floor",
            np.where(df["Rack Locations"] == "Rack", "Rack", "Other")))
    ...
    # Floor/Rack Count
    floor_rack_count = len(df[df["Inventory Type"] == "Floor/Rack"])

    # Other Count
    other_count = len(df[df["Inventory Type"] == "Other"])

    # Write data to new excel file for easier analysis of sorting/cleaning
    metric_df = pd.DataFrame([[invalid_count, floor_count, rack_count, floor_rack_count, other_count]], columns = ["Invalid Entries Quantity", "Floor Locators Quantity", "Rack Locators Quantity", "Floor/Rack Locators Quantity", "Other Entries Quantity"])
